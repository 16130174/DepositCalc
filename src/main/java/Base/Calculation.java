...
public class Calculation {
	...
	public static double Calk( double sk,double s,double st) {
			 return 12*(sk-s)/((st/100)*s);						/** Формула для расчет срока вложения */
		}
	/**
	 * 
	...
	public static double CalcWithoutCap(double s,double p,double sr) { 
			  ...
			  return s+(((s*p*sr)/100)/12);						/** Формула для расчета без капитализации */
		  }
	public static double CalcWithCap(double s,double p,double sr) {			
		
		return s*Math.pow(1+sr*p/12/100, sr);  }				/** Формула для расчета с капитализацией */
