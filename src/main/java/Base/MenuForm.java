...
/** Создаем класс, наследуемый от класса JFrame */
	...
	/** Задаем переменную для слушателя */
	String login="admin";
	/** Задаем значения переменной логина для формы работника */
	String password="2007";
	/** Задаем значения переменной пароля для формы работника */
	String login_kl="user";
	/** Задаем значения переменной логина для формы клиента */
	String password_kl="7000";
	/** Задаем значения переменной пароля для формы клиента */
	...
	public MenuForm() {
		...
		/** Задаем название кнопки-переключателя */
		Rab.setBounds(28, 23, 152, 49);
		/** Задаем расположение и размеры кнопки-переключателя (расположение по x, расположение по y, высота, ширина (в пикселях)) */
		contentPane.add(Rab);
		/** Добавляем кнопку на панель */
		Rab.setBackground(new Color(200,250,250));
		/** Задаем цвет кнопки-переключателя "Работник" */
		...
		/** Задаем цвет кнопки-переключателя "Клиент" */
		ButtonGroup group = new ButtonGroup();
		/** Задаем пременную для группы кнопок
		 														* Для кнопок добавленных в группу будет действовать правило, когда включается одна кнопка, остальные отклчаются */
		group.add(Rab);
		/** Добавляем кнопку "Работник" в группу */
		group.add(Klient);
		/** Добавляем кнопку "Клиент" в группу */
		...
		/** Добавляем слушателя на кнопку */
			...
			/** Создаем обработчик событий */
				...
				/** Условие нажатия кнопки */
					 ...
					 /** Проверка совпадения значения введенного в текстовое поле значения login и заданной переменной */
							if(textField_1.getText( ).equals(password)) {
							/** Проверка совпадения значения введенного в текстовое поле значения password и заданной переменной */
							new Rab().setVisible(true);
							/** Открытие формы калькулятора работника */
							setVisible(false);
							/** Отключаение видимости формы меню */
							}else {
							...
							ExeptionHandler.ErrorMessage("Логин или пароль введены неверно!");
							/** Иначе создается панель сообщающая об ошибке */
						textField.setText("");
						textField_1.setText("");
						/** Поля ввода очищаются*/
						}
				...
				/** Условие нажатия кнопки */
					 ...
					 /** Проверка совпадения значения введенного в текстовое поле значения login и заданной переменной */
							...
							/** Проверка совпадения значения введенного в текстовое поле значения password и заданной переменной */
							new ClientForm().show();
							/** Открытие формы калькулятора клиента */
							setVisible(false);
							/** Отключаение видимости формы меню */
							}else {
								ExeptionHandler.ErrorMessage("Логин или пароль введены неверно!");
